...
M.show_commit = function()
  ...
  local tag_name = string.format("show-jump:commit//%s", commit_hash)
  add_to_tagstack(tag_name)

  local show_buf = vim.api.nvim_create_buf(true, true)
  local show_bufname = string.format("show-jump://%s", commit_hash)
  vim.api.nvim_buf_set_name(show_buf, show_bufname)

  vim.bo[show_buf].filetype = "git"
  -- Set the buffer to be deleted when hidden
  vim.api.nvim_set_option_value("bufhidden", "delete", { buf = show_buf })
